...
**For the team.** ISRO PS 26176 (ORCA), Smart India Hackathon 2026. This document is
the single place to understand the whole build without reading every commit — what
exists, why it exists, how it works, what's honestly missing, and what to do next.
...
1. **A full advisory has to be small enough to survive a connection that barely
   exists** — hence the 145-bit offline capsule (§5).
2. **The system has to be honest about what it knows**, because a wrong "safe to
   sail" verdict has real safety consequences — hence the numeric firewall (§4) and
   the constant discipline of saying "we don't have this data" instead of guessing.

## 3. Who built what

- **Prateek**: Phase 0-2 — toolchain, repo skeleton, and the data plane (source
  registry, Open-Meteo/Copernicus adapters, Zarr cache, PostGIS boundary loader,
  PFZ scraper). 71 tests, all still standing untouched underneath everything below.
- **This build** (on top of that foundation): Phase 3 (risk engine, numeric
  firewall, orchestrator), Phase 4 (145-bit capsule codec), and Phase 5 (the full
  frontend — landing page, Boat surface, Shore Console, Study routes), plus real
  route planning and historical trend analysis that go beyond the original phase
  scope. 151 backend tests + 38 frontend tests on top of Prateek's 71.

## 4. Architecture — how a question becomes an answer
...
│  Planner    │   (keyword-based today — see §8, "no LLM yet")
└─────┬───────┘
...
## 5. The 145-bit offline capsule — the actual differentiator
...
- **Written independently in Python (backend) and TypeScript (frontend)** — not one
  ported from the other. `eval/capsule_corpus.jsonl` is loaded by both languages'
  test suites and both must produce byte-identical results. This caught a real bug:
...
- **Verified live, not just in tests**: a capsule the Python backend generated from
  real Open-Meteo data was pasted into the frontend's decode box and produced the
  correct advisory sentence with zero network calls.

## 6. The numeric firewall — the trust story

"The model may not introduce a number" (this is written as a rule specifically
because an LLM is the eventual narrator — see §8). Today, with no LLM wired in yet,
narration is built from Python string formatting, which can't hallucinate by
construction — but the firewall runs over it anyway, for two reasons: it's the exact
mechanism that will matter once an LLM is added, and it caught **real bugs** during
development:
...
## 7. What's real vs. what's honestly incomplete
...
| Numeric firewall | **Real**, caught real bugs (§6) |
| 145-bit capsule | **Real**, cross-language proven (§5) |
...
| LLM narration | **Not yet** — templated Python strings today; firewall is ready for when one is added |
...
## 9. Repo structure — where this actually lives

This was built on top of **Prateek's original repo**
(`github.com/PRATEEK-260/Dhruva_SIH`), not a fresh project. Since I (Neeyati) don't
have collaborator write access there, the workflow was: fork it to
`github.com/Kalraneeyati/Dhruva_SIH`, push the work to a branch there
(`feature/phase5-frontend`), and open **Pull Request #1 back to Prateek's repo**.
That PR is the real submission — merging it brings everything here into the
original project with full history intact. Nothing was thrown away or rebuilt from
zero; every commit descends from Prateek's original Phase 0-2 work.

PR: `https://github.com/PRATEEK-260/Dhruva_SIH/pull/1`

## 10. Future roadmap, in priority order
...
5. **Merge Prateek's local Phase 3 work** (if his LangGraph-based graph diverges
   from this deterministic dispatch) and bring up the real PostGIS boundary path
   once Docker is available to test it.
...
Every real bug mentioned in §5 and §6 above was caught by either these tests or by
